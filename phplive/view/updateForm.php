...
<main>
    ...
    <h1 class="text-center">Update Your Product Information</h1>
    
		<form method="post" name="frmUpdate" action="">
			<div class="form-group">
				<label for="txtProdID">Product ID (Can't Update ID):</label>
                                <?php 
                                if (isset($_POST["btnUpdateSubmit"]) && !empty($errormsgprodid)){
                                    echo $errormsgprodid;
                                }
                                ?>
				<input type="text" class="form-control bg-faded-4" id="txtProdID" placeholder="Enter Product ID" name="txtProdID" 
                                       value="<?php if(isset($_POST['btnUpdateForm'])) {echo htmlspecialchars($product_id);}
                                       if (isset($_POST['txtProdID']) && empty($errormsgprodid) && empty($errormsgprodn) && empty($errormsgprodprice) && empty($errormsgproddesc)){echo "";}
                                       else if (isset($_POST['txtProdID']) && empty($errormsgprodid)) {echo htmlspecialchars($prod_id);} ?>" readonly>
			...
			<div class="form-group">
				...
				<select id="lstVendorName" name="lstVendorName" class="form-control">
                                    <?php foreach ($vendors as $one_row){ ?>
                                        <option value="<?php 
                                                            echo $one_row["vend_id"]; ?>"
                                            <?php if (!empty($_POST['lstVendorName']) && $_POST['lstVendorName'] == $one_row['vend_id']) { echo 'selected'; } ?>
                                            <?php if (isset($_POST['btnUpdateForm']) && $vendor_id == $one_row['vend_id']) {echo 'selected';} ?>>
			...
			<div class="form-group">
				<label for="txtProdName">Product Name:</label>
                                <?php 
                                if (isset($_POST["btnUpdateSubmit"]) && !empty($errormsgprodn)){
                                    echo $errormsgprodn;
                                }
                                ?>
				<input type="text" class="form-control bg-faded-4" id="txtProdName" placeholder="Enter Product Name" name="txtProdName" 
                                       value="<?php if(isset($_POST['btnUpdateForm'])) {echo htmlspecialchars($product_name);}
			...
			<div class="form-group">
				<label for="txtProdPrice">Product Price:</label>
                                <?php
                                     if (isset($_POST["btnUpdateSubmit"]) && !empty($errormsgprodprice)){
				...
				<input type="text" class="form-control bg-faded-4" id="txtProdPrice" placeholder="Enter Product Price" name="txtProdPrice" 
                                       value="<?php if(isset($_POST['btnUpdateForm'])) {echo htmlspecialchars($product_price);}
			...
			<div class="form-group">
				<label for="txtProdDesc">Product Description (Optional):</label>
                                <?php
                                     if (isset($_POST["btnUpdateSubmit"]) && !empty($errormsgproddesc)){
				...
				<input type="text" class="form-control bg-faded-4" id="txtProdDesc" placeholder="Enter Product Description" name="txtProdDesc" 
                                       value="<?php if(isset($_POST['btnUpdateForm'])) {echo htmlspecialchars($product_description);}
			...
			<div class="form-group text-center">
				...
				<button id="btnUpdateSubmit" name="btnUpdateSubmit" type="submit" value="Update" class="btn btn-primary mt-2">Update</button>
			</div>
		</form>
